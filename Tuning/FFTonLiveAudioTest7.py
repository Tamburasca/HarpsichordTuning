...
"""
A tuning device for string instruments, such as (1) harpsichords and (2) pianos. Collects an audio signal from a
stream of length record_seconds, runs a subsequent FFT with a Hanning apodization and performs Gauss smoothing on
the spectrum. Thereafter we find the highest peaks and its partials.
If at least 5 partials are found the lowest fundamental is determined, compared with a tuning table and the deviation
is displayed.
...
class Tuner:
    ...
    def harmonics(self, ind, height=None):
        ...
        NMAX = 8
        _start = timeit.default_timer()
        # b is the inharmonicity coefficient max displacement 0 < b < 0.005 for a harpsichord
        # see also HARVEY FLETCHER, THE JOURNAL OF THE ACOUSTICAL SOCIETY OF AMERICA VOLUME 36, NUMBER 1 JANUARY 1964
        # ...
        ...
        for i in range(0, len(ind)-1):
            ...
            for j in range(1, len(ind)):
                ...
                for k in range(2, NMAX):  # k is partial of fundamental frequency
